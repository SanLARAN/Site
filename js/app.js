      ...
      list.innerHTML = posts.map((p) => {
        ...
        return `
          <article class="post-card" data-num="${p.number}">
            <div class="post-card-top">${cat ? `<span class="cat-badge" style="background:${cat.color}">${escapeHtml(cat.name)}</span>` : ""}${p.state === "closed" ? `<span class="cat-badge" style="background:#6b7280">закрыт</span>` : ""}</div>
